...
function generateEnv() {
  ...
  const executor = document.querySelector("[data-select=d-executor] .rs-select-value")?.textContent?.trim() || "";
  const provider = document.querySelector("[data-select=d-provider] .rs-select-value")?.textContent?.trim() || "ritual";
  const model = document.querySelector("[data-select=d-model] .rs-select-value")?.textContent?.trim() || "";
  const freq = document.getElementById("d-freq")?.value || "2000";
  const ttl = document.getElementById("d-schedttl")?.value || "500";
  const fund = document.getElementById("d-fund")?.value || "0.25";
  const maxBudget = document.getElementById("d-maxbudget")?.value || "0.5";
  const cliType = document.querySelector("input[name=cli-type]:checked")?.value || "6";
  const env = [
    "# Ritual Sovereign Agent — Generated by Deploy UI",
    ...
    "AGENT_PROMPT=" + prompt.substring(0, 200).replace(/\n/g, " "),
  ].join("\n");
  const output = document.getElementById("env-output");
  const copyBtn = document.getElementById("env-copy-btn");
  if (output) { output.textContent = env; output.classList.remove("hidden"); }
